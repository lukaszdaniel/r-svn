...
attribute_hidden
bool UsingReadline = TRUE;  /* used in sys-std.c & ../main/platform.c
...
attribute_hidden
int R::R_ChooseFile(int _new,  char *buf, int len)
...
/* Tries to ensure that the limit on the number of open files is at least
   ...
   number is returned. */
attribute_hidden int R::R_EnsureFDLimit(int desired) {
