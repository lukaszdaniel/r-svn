...
static bool GetGrayPalette(Display *displ, Colormap cmap, int n)
{
    int status;
    int m = 0;
    for (int i = 0; i < n; i++) {
...
static void SetupGrayScale(void)
{
	...
	warning("%s", _("cannot set grayscale: reverting to monochrome"));
	model = MONOCHROME;
	SetupMonochrome();
    }
}
...
static void SetupPseudoColor(void)
{
	...
	if (PaletteSize == 0) {
	    warning("%s", _("X11 driver unable to obtain color cube\n  reverting to monochrome"));
	    model = MONOCHROME;
	    SetupMonochrome();
	}
    }
...
static unsigned int GetPseudoColor2Pixel(int r, int g, int b)
{
	...
	error("%s", _("Error: X11 cannot allocate additional graphics colors.\n\
...
static bool SetupX11Color(void)
{
    if (depth <= 1) {
...
static void *RLoadFont(pX11Desc xd, char* family, int face, int size)
{
	...
	if ( ADOBE_SIZE(pixelsize) ) {
		...
		error("%s", _("could not find any X11 fonts\nCheck that the Font Path is correct."));
	}
    ...
    {
	for (int i = 0 ; i < CLRFONTS ; i++)
	      R_XFreeFont(display, fontcache[i].font);
	for (int i = CLRFONTS ; i < MAXFONTS ; i++)
...
static void SetFont(const pGEcontext gc, pX11Desc xd)
{
	...
	R_XFont *tmp = (R_XFont*) RLoadFont(xd, family, face, size);
	if(tmp) {
	    xd->font = tmp;
...
bool X11_Open(pDevDesc dd, pX11Desc xd, const char *dsp,
	 ...
	 int xpos, int ypos)
{
	...
	*/
	...
	warning("%s", _("ignoring 'display' argument as an X11 device is already open"));
    whitepixel = GetX11Pixel(R_RED(canvascolor), R_GREEN(canvascolor),
			     R_BLUE(canvascolor));
    blackpixel = GetX11Pixel(0, 0, 0);
#ifdef HAVE_WORKING_X11_CAIRO
    if(xd->useCairo && Vclass != TrueColor) {
	warning("%s", _("cairo-based types may only work correctly on TrueColor visuals"));
    }
#endif
	...
	warning("%s", _("jpeg() does not support transparency: using white bg"));
	xd->canvas = 0xffffff;
    }
	...
	if(alreadyCreated == 0) {
	    ...
	    if (xd->window == 0 ) {
	      XFree(hint);
	      warning("%s", _("unable to create X11 window"));
	      return false;
	    }
	...
	if ((xd->window = XCreatePixmap(
	    ...
	    warning("%s", _("unable to create pixmap"));
	    return false;
	}
	/* Save the pDevDesc with the window for event dispatching */
	/* Is this needed? */
...
static char *SaveFontSpec(SEXP sxp, int offset)
{
    ...
    char *s = R_alloc(strlen(CHAR(STRING_ELT(sxp, offset)))+1, sizeof(char));
...
static char* translateFontFamily(char* family, pX11Desc xd)
{
    CXXR::GCStackRoot<> graphicsNS, fontdb, fontnames;
...
#ifdef DEBUG_X11
	    int cnt = XFontsOfFontSet(xd->font->fontset, &fs_list, &ml);
                      ...
                      const pGEcontext gc, pDevDesc dd)
{
    ...
    if (image == NULL || XInitImage(image) == 0)
        error("%s", _("Unable to create XImage"));
...
static void X11_eventHelper(pDevDesc dd, int code)
{
    ...
    if (code == 1) {
    	...
    	if (isEnvironment(dd->eventEnv)) {
    	    CXXR::GCStackRoot<> prompt;
    	    prompt = findVar(install("prompt"), dd->eventEnv);
...
bool X11DeviceDriver(pDevDesc dd,
...
static SEXP in_do_saveplot(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (!gdd->dirty) error("%s", _("no plot on device to save"));
    xd = (X11Desc*) gdd->dev->deviceSpecific;
    if (!xd->cs || !xd->useCairo) error("%s", _("not an open X11cairo device"));
    if (streql(type, "png")) {
...
static SEXP in_do_saveplot(SEXP call, SEXP op, SEXP args, SEXP env)
{
    error("%s", _("savePlot() is not supported on this build"));
    return R_NilValue;
}
#endif
...
static Rboolean in_R_X11readclp(Rclpconn this_, const char *type)
{
    ...
    if (!displayOpen) {
	if ((display = XOpenDisplay(NULL)) == NULL) {
	    warning("%s", _("unable to contact X11 display"));
	    return FALSE;
	}
    }
...
void R_init_R_X11(DllInfo *info)
{
    ...
    if(!tmp) {
	error("%s", _("cannot allocate memory for X11Routines structure"));
	return;
    }
    tmp->X11 = in_do_X11;
