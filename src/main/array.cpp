...
attribute_hidden
R_xlen_t R::dim2total(SEXP dim /* INTSXP */, const char *ErrMsg)
{
    int ndim = LENGTH(dim);
    if (ndim == 0)
...
attribute_hidden SEXP do_colsum(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    case CPLXSXP: break;
    default:
	error(_("'%s' must be numeric or complex"), "x");
    }
...
#pragma omp parallel for num_threads(nthreads) default(none) \
	...
	for (R_xlen_t j = 0; j < p; j++) {
		...
		Complex csum = { double(sum), double(i_sum) };
		COMPLEX(ans)[j] = csum;
		continue; /* for(j ...) loop */
	    }
	    default:
	...
	int *Cnt = NULL;
	LDOUBLE *rans = nullptr;
	LDOUBLE *ians = nullptr; // unused unless CPLXSXP
	if(n <= 10000) {
	...
	if(type == CPLXSXP) {
	    for (R_xlen_t i = 0; i < n; i++) {
		Complex csum = { double(rans[i]), double(ians[i]) };
		COMPLEX(ans)[i] = csum;
	    }
	} else {
	    for (R_xlen_t i = 0; i < n; i++)
