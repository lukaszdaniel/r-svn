	           ...
	           int *rlen, int assumevalid)
{
	...
	if (!assumevalid && !utf8Valid(str)) {
	    error(_("invalid multibyte string, element %lld"), (long long)idx+1);
	}
          ...
          R_xlen_t xidx, R_xlen_t vidx)
{
    ...
    if (ienc == CE_UTF8) {
	if (!utf8Valid(buf)) {
	    error(_("invalid multibyte string, element %lld"), (long long)xidx+1);
	}
	if (!utf8Valid(str)) {
	    error(_("invalid multibyte string, value element %lld"), (long long)vidx+1);
	}
...
attribute_hidden SEXP do_substrgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	for (R_xlen_t i = 0; i < len; i++) {
		...
		*/
		...
		char* buf = (char *) R_AllocStringBuffer(slen+strlen(v_ss), &cbuff);
		strcpy(buf, ss);
