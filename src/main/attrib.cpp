...
void Rf_copyMostAttrib(SEXP inp_, SEXP ans_)
{
    if (ans_ == R_NilValue)
	error("%s", _("attempt to set an attribute on NULL"));

    GCStackRoot<> inp(inp_), ans(ans_);
    for (SEXP s = ATTRIB(inp); s != R_NilValue; s = CDR(s)) {
...
void R::copyMostAttribNoTs(SEXP inp_, SEXP ans_)
{
    if (ans_ == R_NilValue)
	error("%s", _("attempt to set an attribute on NULL"));
...
static SEXP installAttrib(SEXP vec, SEXP name, SEXP val)
{
    ...
    for (SEXP s = ATTRIB(vec); s != R_NilValue; s = CDR(s)) {
        ...
        prev = s; // record last attribute, if any
    }
    ...
    if (ATTRIB(vecrt) == R_NilValue) SET_ATTRIB(vecrt, s); else SETCDR(prev, s);
...
attribute_hidden SEXP R::R_S4_extends(SEXP klass, SEXP useTable)
{
...
{
    for (unsigned int type = 0; type < MAX_NUM_SEXPTYPE; type++) {
	GCStackRoot<> part3(R_NilValue);
	GCStackRoot<> part4(R_NilValue);

	switch(type) {
	    case CLOSXP:
	    case SPECIALSXP:
	    case BUILTINSXP:
		part3 = mkChar("function");
		break;
	    case INTSXP:
	    case REALSXP:
		part3 = type2str_nowarn((SEXPTYPE) type);
		part4 = mkChar("numeric");
		break;
	    case LANGSXP:
...
SEXP Rf_dimnamesgets(SEXP vec_, SEXP val_)
{
	...
	int i = 0;
	for (val = vec; !isNull(val); val = CDR(val.get()))
...
attribute_hidden SEXP do_attrgets(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	if (XLENGTH(name) > 1)
	    error("%s", _("'name' must be a scalar string"));
...
attribute_hidden SEXP R::S3Class(SEXP obj)
{
