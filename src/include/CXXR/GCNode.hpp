...
namespace CXXR
{
    ...
    struct sxpinfo_struct {
        sxpinfo_struct(SEXPTYPE stype = NILSXP): type(stype), scalar(false), obj(false),
            ...
            gccls(0), m_refcnt(0), m_binding_tag(NILSXP), m_infant(true), extra(0)
        {
        }
    ...
    public:
         ...
         * existence.
         */
        static size_t numNodes() { return s_GCNodeTable.size(); }
...
// #define EXPEL_OLD_TO_NEW
        using Table = std::unordered_set<const GCNode *, std::hash<const GCNode *>, std::equal_to<const GCNode *>,
            Allocator<const GCNode *>>;
        static Table s_for_deletion;
        static Table s_GCNodeTable;
#ifndef EXPEL_OLD_TO_NEW
        static Table s_OldToNew[GCNode::s_num_old_generations];
#endif
