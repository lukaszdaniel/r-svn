...
void QuartzBitmap_Output(QuartzDesc_t dev, QuartzBitmapDevice *qbd)
{
	...
	snprintf(buf, R_PATH_MAX, qbd->path, qbd->page); buf[R_PATH_MAX] = '\0';
        ...
        if (!path)
            error("%s", _("QuartzBitmap_Output - filename is empty!"));
...
QuartzDesc_t QuartzBitmap_DeviceCreate(void *dd, QuartzFunctions_t *fn, QuartzParameters_t *par)
{
	...
	if(dev == NULL) error("%s", _("allocation failure in QuartzBitmap_DeviceCreate"));
	...
	if(type) { // code above forces this
	    ...
	    if(dev->uti == NULL) {
		free(dev);
		error("%s", _("allocation failure in QuartzBitmap_DeviceCreate"));
	    }
	} else dev->uti = NULL;
