...
int pureNullUnit(SEXP unit, int index, pGEDevDesc dd) {
	...
	if (unitUnit(unit, index) == L_GROBWIDTH) {
	    ...
	    width = Rf_eval_with_gd(R_fcall2, R_gridEvalEnv, dd);
	    ...
	    height = Rf_eval_with_gd(R_fcall2, R_gridEvalEnv, dd);
...
SEXP summaryUnits(SEXP units, SEXP op_type) {
	...
	std::unique_ptr<int[]> tmp = std::make_unique<int[]>(m > 0 ? m : 1L); // m might be zero which would not be legal C.
	int *is_type = tmp.get();
	int all_type = 1;
