...
static SEXP baseCallback(GEevent task, pGEDevDesc dd, SEXP data)
{
	...
	bss = (baseSystemState *) (sd->systemSpecific);
	...
	bss2 = (baseSystemState *) (curdd->gesd[baseRegisterIndex]->systemSpecific);
	...
	if (isReal(data) && LENGTH(data) == 1) {
	  ...
	  error("%s", _("event 'GE_ScalePS' requires a single numeric (double) value"));
	break;
    }
    }
    return result;
