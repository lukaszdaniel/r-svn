...
#include <memory>
#include <cstring> // for memset, strcat, strlen, strncmp
...
SEXP modelframe(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	row_names = allocVector(INTSXP, (nr > 0) ? 2 : 0);
	if (nr > 0) {
...
SEXP modelmatrix(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	error("%s", _("invalid model frame"));
    if (length(vars) == 0)
	error("%s", _("do not know how many cases"));
...
/*	else
 ...
 alldone:
    ...
    for (j = 0; j < nterms; j++) {
	if (j == rhs_response) {
	    warning("%s", _("the response appeared on the right-hand side and was dropped"));
	...
	for (kk = 0; kk < INTEGER(count)[j]; kk++) {
		...
		if (ll) {
			...
			if(strlen(buf) + strlen(addp) < BUFSIZE)
			    ...
			    warning("%s", _("term names will be truncated"));
			if (x == R_NilValue) {
			...
			error("%s", _("complex variables are not currently allowed in model matrices"));
...
static SEXP ExpandDots(SEXP object, SEXP value)
{
 ...
 badformula:
    error("%s", _("invalid formula in 'update'"));
...
{
    R_xlen_t n = 0;
    for (SEXP cl = form; cl != R_NilValue; cl = CDR(cl), n++) {
	Rprintf(" L[%td] (|L[.]|=%d): ", n+1, length(CAR(cl)));
	printVector(CAR(cl), 0, 0);
    }
    return;
}
static bool trace_GetBit = true;
#define trace_InstallVar 1;
...
static int InstallVar(SEXP var)
{
	...
	error("%s", _("invalid term in model formula"));
    /* Lookup/Install it */
    int indx = 0;
	...
	Rprintf("InstallVar(%s): adding it to 'varlist' and return indx+1 = %d\n",
		CHAR(STRING_ELT(deparse1line(var, FALSE), 0)), indx + 1);
#endif
...
static void ExtractVars(SEXP formula)
{
    ...
    Rprintf("%*d ExtractVars(%s): ", 2*n_xVars, n_xVars,
	    CHAR(STRING_ELT(deparse1line(formula, FALSE), 0)));
...
#  define Return  return
	...
	if (CAR(formula) == powerSymbol) {
		...
		error("%s", _("invalid power in formula"));
	    ExtractVars(CADR(formula));
	    Return;
	}
...
SEXP termsform(SEXP args)
{
	...
	error("%s", _("argument is not a valid model"));
	...
	error("%s", _("'specials' must be NULL or a character vector"));
    SEXP
	a = CDDR(args),
    ...
    Rprintf("termsform(): calling ExtractVars(CAR(args), 1), where\n CAR(args)='%s'\n",
	    CHAR(STRING_ELT(deparse1line(CAR(args), FALSE), 0)));
#endif
	...
	for (j = 0, t = t_; j < i; j++, t = CDR(t)) {
		...
		if (TYPEOF(call) == LANGSXP && TYPEOF(CAR(call)) == SYMSXP &&
		    streql(CHAR(PRINTNAME(CAR(call))), ss))
		    k++;
	    }
	    if (k > 0) {
